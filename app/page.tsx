...
import { Menu, Loader2, ChevronLeft, ChevronRight, X, Image as ImageIcon, AlertCircle } from "lucide-react";
...
interface TooltipProps {
  children: ReactNode; // ReactNode is the correct type for anything renderable in JSX
...
const Dashboard = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <button
  onClick={toggleNav}
  className="absolute top-4 right-4 md:hidden z-40 p-2 rounded-full bg-blue-500 text-white shadow-lg hover:bg-blue-600"
>
  ...
  initial={false}
  animate={{
    width: isNavOpen ? "16rem" : "5rem", // 64px (w-16) to 256px (w-64)
    x: isNavOpen ? 0 : window.innerWidth < 768 ? -80 : 0,
  }}
  transition={{
    type: "spring", // Smooth spring animation
    stiffness: 120, // Controls the "bounciness" of the animation
    damping: 20, // Reduces oscillation for a smoother stop
  }}
  className={`fixed md:relative z-30 h-full bg-white shadow-lg transition-all duration-300 ease-in-out flex-shrink-0
    ${isNavOpen ? "translate-x-4" : "-translate-x-full md:translate-x-0"}`}
>
  {/* Toggle Button */}
  <button
    onClick={() => setIsNavOpen(!isNavOpen)}
    className="absolute -right-3 top-4 bg-white rounded-full p-1 shadow-md z-10 md:flex hidden"
  >
  ...
  <div className="max-w-4xl mx-auto mt-16 md:mt-0">
    ...
    >
      {/* Flex container for inputs and image */}
